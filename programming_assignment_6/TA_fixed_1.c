...
void addFirst(LinkedList *someList, void *newElement){
  ...
  temp->prev = header;
  header->next = temp;
  // Missing connect from old front node prev to new front node
...
void *removeLast(LinkedList *someList) {
  ...
  if(someList == NULL){
    ...
    // it should point to the header of someList
    newLastNode->next = someList->header;
    temp = lastNode->data;
    //newLastNode->next = lastNode->next;
    free(lastNode);
    //lastNode = newLastNode;
    // This shouldn't be commented
    someList->size--;
    //printf("%s\n", (char *) secondLast->data);
  }
  /*printf("Old data: %s\n", (char *) temp);*/
  return temp; 
  //return NULL;
...
void *set(LinkedList *someList, int position, void *newElement){
  // This is incorrect. The position 0 is 
  // someList->header->next
  // Position 1 is someList->header->next->next
  // and so on
  ...
  // Jose: this is wrong, you're not supposed
  // to add a new node, just replace the data
  // record of identified node in position 'position' 
  // (in this case front), change the value to 'newElement'
  // and return the previous value in 'front'
