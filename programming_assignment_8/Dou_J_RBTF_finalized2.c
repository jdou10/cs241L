...
bool isHeapOrdered(Node *curr){
    int higher_left = (curr->data && curr->left->data);
    int higher_right = (curr->data && curr->right->data);
    if((higher_left <= curr->data) || (higher_right <= curr->data)){
        return false;
    }
    if(curr->left == NULL && higher_right){
        return true;
    }
    if(curr->right == NULL && higher_left){
        return true;
    }
    if(curr == NULL){
        return true;
    ...
    check_left = isHeapOrdered(curr->left);
    check_right = isHeapOrdered(curr->right);
    if((check_left && check_right) == 1){
        return false;
    }else{
        return true;
    }
...
int numSingleChild(Node *curr){
    ...
    if((curr->left != NULL && curr->right == NULL) ||
       (curr->left == NULL && curr->right != NULL)){
        ...
        int size_combine, height_combine;
        size_combine = size(curr->left) + size(curr->right);
        height_combine = height(curr->left) + height(curr->right);
        if(size_combine != height_combine){
            int combine;
            combine = size_combine - height_combine - 1;
            printf("%d\n", combine);
        }
    }
