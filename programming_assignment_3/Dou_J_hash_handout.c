...
/* Dou Jingru
CS 241
hash_handout.c is to hash a string in the program, while running the program,
the input only can be one word otherwise will not compile.
Type ./a.out alice and the program will print out the hash value of alice which is 102.
 */
...
int main( int argc, char *argv[] ) {
  ...
  for ( i = 0; i < strlen(theData); i++ ) {
    ...
    hashCode = nextChar + 1;
    leftMostFourBits = hashCode << 4;
    leftMostFourBits = leftMostFourBits & 3932160; //2^
